...
public class ProjectServices
{
    ...
    public ProjectServices(IMapper mapper, ApplicationDbContext context)
    {
        _mapper = mapper;
        _context = context;
    }
    ...
    public async Task<ApiResponse<string>> AddAsync(ProjectDto projectDto)
    {
        ...
        if (await _context.SaveChangesAsync() > 0)
            return ApiResponse<string>.Error(500, "Failed To Add Project");
    ...
    public async Task<ApiResponse < IEnumerable< ProjectDto>>> GetAllAsync()
    {
        ...
        if(ProjectsDto == null)
            return ApiResponse<IEnumerable<ProjectDto>>.Error(500, "Not Data Found");
    ...
    public async Task<ApiResponse<ProjectDto>> GetByIdAsync(int id, ProjectDto projectDto)
    {
        ...
        if (Project is null)
            return  ApiResponse<ProjectDto>.Error(404,"Not found");
    ...
    public async Task<ApiResponse<string>> UpdateAsync(int id, ProjectDto projectDto)
    {
        ...
        if (ExistsProject is null)
            return  ApiResponse<string>.Error(404,"Project Not Found");

        var ProjectDto = _mapper.Map(projectDto, ExistsProject);

         _context.Projects.Update(ProjectDto);
        if (await _context.SaveChangesAsync() > 0)
            return  ApiResponse<string>.Error(404,"Failed To Update Project");
    ...
    public async Task<ApiResponse<string>> DeleteAsync(int id)
    {
        ...
        if ( _context.SaveChanges()>= 0)
            return ApiResponse<string>.Error(404,"Failed To Delete Project");
